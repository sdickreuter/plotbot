...
#  al_a = 815.0 # a arm length [mm]
#  al_b = 815.7#85.5 #  arm length [mm]
#  fa = 800.0 # y position of rail a [mm]
#  fb = -30.0 # y position of rail b [mm]
#  zero = vec2(701.4271166700073, 415.0) # x/y pos when position on rail a/b is 0.0/0.0
#  para_offset = 45.0 # [mm] offset along/parallel arm a
#  ortho_offset = 30.0 # [mm] offset away from/orthogonal arm a
...
proc xy_to_ab*(p: var Path) =
  for i in 0..<len(p.c):
    case p.c[i].kind
      ...
      of ckBezier:
        p.c[i] = get_cubic_bezier_from_points(p.c[i].b0.xy_to_ab(), p.c[
            i].calc_point_t(1/3).xy_to_ab()
  ...
  for i in 0..<len(p.c):
    case p.c[i].kind
      of ckLine:
        echo("WARNING in coordtransform.nim: can't transform lines, convert lines to cubic beziers before transformation!")
      of ckBezier:
        p.c[i] = get_cubic_bezier_from_points(p.c[i].b0.ab_to_xy(), p.c[
            i].calc_point_t(1/3).ab_to_xy()
